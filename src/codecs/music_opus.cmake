...
if(USE_OPUS)
    ...
    if(USE_SYSTEM_AUDIO_LIBRARIES)
        ...
        message("OpusFile: [${OpusFile_FOUND}] ${OpusFile_INCLUDE_DIRS} ${OpusFile_LIBRARIES}")
        if(USE_OPUS_DYNAMIC)
            list(APPEND SDL_MIXER_DEFINITIONS -DOPUS_DYNAMIC=\"${OpusFile_DYNAMIC_LIBRARY}\")
            message("Dynamic OpusFile: ${OpusFile_DYNAMIC_LIBRARY}")
        endif()
    else()
        if(DOWNLOAD_AUDIO_CODECS_DEPENDENCY)
            set(OpusFile_LIBRARIES opusfile${MIX_DEBUG_SUFFIX} opus${MIX_DEBUG_SUFFIX})
        else()
            find_library(LIBOPUSFILE_LIB NAMES opusfile
                         HINTS "${AUDIO_CODECS_INSTALL_PATH}/lib")
            find_library(LIBOPUS_LIB NAMES opus
                         HINTS "${AUDIO_CODECS_INSTALL_PATH}/lib")
            set(OpusFile_LIBRARIES ${LIBOPUSFILE_LIB} ${LIBOPUS_LIB})
        endif()
        set(OpusFile_FOUND 1)
    ...
    if(OpusFile_FOUND)
        ...
        if(NOT USE_SYSTEM_AUDIO_LIBRARIES OR NOT USE_OPUS_DYNAMIC)
            list(APPEND SDLMixerX_LINK_LIBS ${OpusFile_LIBRARIES} ${LIBOPUS_LIB})
            set(LIBOGG_NEEDED ON)
            set(LIBMATH_NEEDED ON)
        endif()
        list(APPEND SDL_MIXER_INCLUDE_PATHS ${OpusFile_INCLUDE_DIRS})
